...
$$\Omega(N,q) = \begin{pmatrix}(N-1)+ q \\ q\end{pmatrix} = \frac{(N-1+q)!}{q!(N-1)!}.$$
...
Using the [[Second law of thermodynamics]], we could deduce from this plot that the equilibrium of [[Einstein crystal|these crystals]] in contact with each other is reached when the energy is shared equally between them - since that is when the [[Multiplicity|multiplicity]] (and thus also the [[Entropy|entropy]]) is greatest. This corresponds well with our understanding of how temperature spreads from one solid to another.
