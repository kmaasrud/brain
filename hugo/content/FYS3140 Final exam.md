...
which is a [Gaussian integral]({{< ref "Gaussian integral.md" >}}).
## 1.e)
...
which gives the [Fröbenius]({{< ref "Fröbenius.md" >}}) solution as
$$y(x) = \sum_{n=0}^N(-1)^n\frac{N!}{(N - n)!(n!)^2}a_0x^n.$$
# Problem 3
## 3.a)
$$f(x)$$ is an [Odd function]({{< ref "Odd function.md" >}}), which means there will be no cosines in the series (and also no $$a_0$$, since the function is not defined in $$x=0$$). 
...
and so the [Fourier series]({{< ref "Fourier series.md" >}}) of the function is:
...
Since the integrand $$\frac{1-\cos k}{k^2}$$ is an even function, I utilize the [symmetry]({{< ref "Parity.md" >}}) of it to split the integral into two equal integrals from $$0$$t to $$\infty$$:
$$2 = \frac{2}{\pi}\cdot 2\int^\infty_0\frac{1-\cos k}{k^2}dk$$
$$\Rightarrow \int^\infty_0\frac{1 - \cos k}{k^2}dk = \frac{\pi}{2}$$
# Problem 5
## 5.a)
I start by [Laplace transforming]({{< ref "Laplace transform.md" >}}) the [PDE]({{< ref "Partial differential equation.md" >}}) with regards to the time dependence:
...
Renaming $$\mathcal L\left\{u(x,t)\right\} = U(x,s)$$, I now end up with the following [ODE]({{< ref "Ordinary differential equation.md" >}}):
...
## 5.b)
The [Laplace transform]({{< ref "Laplace transform.md" >}}) $$U(x,s)$$ has to apply to the same boundary conditions as $$u(x,t)$$ (with regards to $$x$$ of course). Applying them will give me expressions for $$A(s)$$ and $$B(s)$$.
...
as the boundary condition for $$u(0,t)$$, the [Laplace transform]({{< ref "Laplace transform.md" >}}) $$G(s) = \mathcal L\{g(t)\}$$ is the boundary condition for $$U(0,s)$$. Thus,
$$U(0, s) = B(s) = G(s),$$
